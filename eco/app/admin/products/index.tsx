...
export default function AdminProducts() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Falha no fetch de produtos", error);
      Toast.show({
        type: "error",
        text1: "Falha ao carregar produtos",
        text2:
  ...
  const performDelete = async (id: string) => {
    try {
      ...
      console.log("Falha na deleção de produto", error);
      Toast.show({
        type: "error",
        text1: "Falha ao deletar produto",
        text2:
          error.response?.data?.message ||
          "Por favor, tente novamente mais tarde",
      });
